...
function CountdownUnit({ value, label, isFinal }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div className="relative group">
        ...
        <div className="relative flex gap-1.5">
          {digits.split('').map((d, i) => (
            <div
              key={i}
              className={`w-[52px] h-[72px] max-sm:w-[40px] max-sm:h-[56px] bg-white/[0.08] backdrop-blur-xl rounded-xl border flex items-center justify-center text-[2.2rem] max-sm:text-[1.6rem] font-bold text-white font-heading tabular-nums shadow-[inset_0_1px_0_rgba(255,255,255,0.1),0_8px_32px_rgba(0,0,0,0.3)] transition-all duration-300 ${
                isFinal
      ...
      <span className="text-[0.7rem] font-semibold tracking-[0.25em] uppercase text-white/50">{label}</span>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center overflow-hidden" style={{ background: '#030014' }}>
      ...
      <div
        ...
        style={{
          width: phase >= 1 ? '200vw' : '0px',
          height: phase >= 1 ? '200vw' : '0px',
      ...
      <div
        ...
        style={{
          width: phase >= 1 ? '180vw' : '0px',
          height: phase >= 1 ? '180vw' : '0px',
      ...
      <div
        ...
        style={{
          width: phase >= 2 ? '300px' : '8px',
          height: phase >= 2 ? '300px' : '8px',
      ...
      {phase >= 1 && Array.from({ length: 20 }).map((_, i) => {
        ...
        const distance = phase >= 2 ? 300 + Math.random() * 200 : 0;
      ...
      <div className="relative z-10 flex flex-col items-center gap-6">
        ...
        >
          <h2 className="text-[3.5rem] max-sm:text-[2.2rem] font-heading font-extrabold text-white leading-none tracking-tight">
            We&apos;re <span className="bg-gradient-to-r from-[#4353ff] via-[#8b5cf6] to-[#06b6d4] bg-clip-text text-transparent">Live</span>
          </h2>
          <p className="text-white/50 text-sm font-medium">Welcome to the future of enterprise AI</p>
        </div>

        {/* Loading bar */}
        <div
          className="w-48 h-1 bg-white/10 rounded-full overflow-hidden transition-all ease-out"
          style={{
            opacity: phase >= 3 ? 1 : 0,
...
export default function LaunchGate({ children }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] rounded-full animate-[float_8s_ease-in-out_infinite] transition-all duration-1000"
        style={{
      ...
      />
      <div
        className="absolute bottom-[-20%] right-[-10%] w-[500px] h-[500px] rounded-full animate-[float_10s_ease-in-out_infinite_reverse] transition-all duration-1000"
        style={{
      ...
      />
      <div
        className="absolute top-[30%] right-[20%] w-[300px] h-[300px] rounded-full animate-[float_12s_ease-in-out_infinite] transition-all duration-1000"
        style={{
      ...
      {isFinalCountdown && (
        <div className="absolute w-[400px] h-[400px] rounded-full border border-[#4353ff]/30 animate-ping" style={{ animationDuration: '1.5s' }} />
      )}
      ...
      <div className="relative z-10 flex flex-col items-center gap-12 max-sm:gap-8 px-6 max-w-[680px] w-full">
        ...
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <h2 className="text-[3rem] max-md:text-[2.2rem] max-sm:text-[1.6rem] font-heading font-extrabold text-white leading-[1.1] tracking-tight">
            {isFinalCountdown ? (
              <>
                Get<br />
          ...
          <p className="text-white/40 text-sm max-w-[400px] leading-relaxed font-medium">
            {isFinalCountdown
        ...
        {/* Countdown */}
        <div className={`flex items-center gap-6 max-sm:gap-3 transition-transform duration-500 ${isFinalCountdown ? 'scale-110' : 'scale-100'}`}>
          ...
          <span className="text-white/20 text-2xl font-light mt-[-24px]">:</span>
          <CountdownUnit value={timeLeft.minutes} label="Minutes" isFinal={isFinalCountdown} />
          <span className="text-white/20 text-2xl font-light mt-[-24px]">:</span>
          <CountdownUnit value={timeLeft.seconds} label="Seconds" isFinal={isFinalCountdown} />
        </div>
        ...
        <div className="w-full max-w-[400px] flex flex-col gap-2">
          ...
          <div className="flex justify-between">
            <span className="text-[0.65rem] text-white/25 font-medium">Building</span>
            <span className="text-[0.65rem] text-white/25 font-medium">Launch Day</span>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-4 mt-4">
          <div className="flex items-center gap-6">
            <a href="mailto:hello@zenvict.com" className="text-white/30 text-xs font-medium hover:text-white/60 transition-colors no-underline">hello@zenvict.com</a>
            <span className="w-px h-3 bg-white/10" />
            <span className="text-white/30 text-xs font-medium">zenvict.com</span>
          </div>
          <p className="text-white/15 text-[0.65rem]">&copy; 2026 Zenvict Technologies. All rights reserved.</p>
        </div>
      </div>
    </div>
  );
